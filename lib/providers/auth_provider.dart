...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _token = responseData['token']; // Stocke le token en mémoire
        await _secureStorage.saveToken(_token!); // Sauvegarde le token sécurisé
        notifyListeners();
  ...
  Future<bool> register(String firstName, String lastName, String email, String password) async {
    ...
    final body = jsonEncode({
      'prenom': firstName,  // Vérifie que les clés sont bien celles de ton API
      'nom': lastName,
      'email': email,
      'password': password,
    });
  ...
  Future<void> logout() async {
    _token = null;
    await _secureStorage.deleteCredentials(); // ✅ Supprime tout (email, password, token)
    notifyListeners();
